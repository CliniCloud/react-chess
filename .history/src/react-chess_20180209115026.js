...
class Chess extends React.Component {
  ...
  handleDragStop(evt, drag) {
    ...
    this.setState({dragFrom: null, targetTile: null, draggingPiece: null, pieceNextMov:null, attacks:null})
  ...
  isSameTeamPiece(draggingPiece, dragToPos){
      const positionedPiece = utils.findPieceAtPosition(this.props.pieces, dragToPos)
    return positionedPiece && utils.isPiecesFromSameTeam(draggingPiece, positionedPiece)
  }
  ...
  renderNextMovs(x, y, nextMovPos, attacks){
    if(nextMovPos && nextMovPos.x === x && nextMovPos.y === 7 - y){
        console.log('nextMovPos.x', nextMovPos.x, x);
        console.log('nextMovPos.y', nextMovPos.y, y);
        return <div className="possible-moviments"></div>
    }else if(attacks && attacks.length){
  ...
  render() {
    const {targetTile, draggingPiece, boardSize, attacks, pieceNextMov} = this.state
      ...
      for (let x = 0; x < 8; x++) {
        ...
        tiles.push(
          <div key={`rect-${x}-${y}`} style={styles}>
            ...
            {this.renderNextMovs(x, y, pieceNextMov, attacks)}
          </div>
        )
      }
    }
