...
module.exports.getPawnOptions = function(pieces, piece) {
    ...
    const nextMov = this.createNewPieceByNotation(pieces, `${piece.name}@${frontPosition}`, piece.index)
    console.log('old position', nextMov);


    const leftAttack = this.getPawAttack(pieces, nextMov, +1 )
    console.log('----------------LEFT newAttack ----------', leftAttack);
    const rightAttack = this.getPawAttack(pieces,nextMov , -1)
    console.log('----------------RIGHT newAttack ----------', rightAttack);

    return { nextMov, leftAttack, rightAttack }
