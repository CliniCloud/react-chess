...
module.exports.getPawnOptions = function(pieces, piece) {
    ...
    const piecePositions = this.createNewPieceByNotation(`${piece.name}@${frontPosition}`, piece.index)
    console.log('old position', piecePositions);
    // var newX = piecePositions.x + positionInc


    // let newAttack = this.getPawAttack(pieces,Object.assign( piece,piecePositions, {x:piecePositions.x - 1}))
...
module.exports.createNewPieceByNotation = function(notation, index) {
...
module.export.createNewPiece = function(name, index, position) {
    return {
