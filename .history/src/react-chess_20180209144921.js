...
class Chess extends React.Component {
        ...
        if (attacks && attacks.length) {
            for(const attack of attacks){
                if(attack && attack.x === x && attack.y === y){
                    return (<Piece isMoving={ isMoving } x={ x } y={ y } fill="yellow"/>)
                }
            }
        }
    ...
    render() {
        ...
        const pieces = this.props.pieces.map((decl, i) => {
            ...
            return (
                <Draggable bounds="parent" position={ { x: 0, y: 0 } } onStart={ this.handleDragStart } onDrag={ this.handleDrag } onStop={ this.handleDragStop } key={ `${piece}-${x}-${y}` } style={{background:'red'}}>
